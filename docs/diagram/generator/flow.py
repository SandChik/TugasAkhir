...
def alir(nama, fn, png, lanes, steps, edges, catatan=None):
    ...
    for ed in edges:
        a, b, lbl, mode = ed[:4]; ldy = ed[4] if len(ed) > 4 else 0   # ldy: geser label koridor
        ...
        if lbl and mode in ("koridor", "koridor_kiri"):
            w, h = text_size(lbl, 8)
            kx = pts[1][0]; yl = (cya-60 if cyb < cya else cya+60) + ldy
...
alir("Gambar IV.3 Alur Sistem Diusulkan Bagian 1", "IV-03-alur-sistem-diusulkan-bagian-1.drawio", "iv3.png",
 ...
 {"m": (0, 70, 44, "Mulai", "term"),
  ...
  "c1":(1, 510, 70, "Memetakan baris menjadi calon\nkegiatan dan mencocokkan\nnama dosen ke akun", "task"),
...
alir("Gambar IV.4 Alur Sistem Diusulkan Bagian 2", "IV-04-alur-sistem-diusulkan-bagian-2.drawio", "iv4.png",
 ...
 {"ka":(0, 60, 60, "A", "conn"),
  ...
  "s1":(1, 390, 75, "Memeriksa keaslian bukti\nmelalui layanan model bahasa\nvisual dan menandai temuan", "task"),
  "s2":(1, 515, 60, "Mencatat jejak dokumen bukti\nke kontrak registri", "task"),
  "x1":(3, 515, 70, "BKDDokumenRegistri\nevent DokumenTercatat", "task"),
  "d3":(0, 630, 60, "Menyimpan permanen\ndokumen BKD", "task"),
  "a1":(2, 740, 75, "Memverifikasi kesesuaian\nparameter terhadap dokumen\nsumber dan menetapkan status", "task"),
  "a2":(2, 865, 95, "Seluruh kegiatan\ndisetujui?", "dec"),
  "a3":(2, 1010, 65, "Mengembalikan kegiatan\nuntuk diperbaiki", "task"),
  "d4":(0, 1010, 60, "Memperbaiki dokumen\nbukti kegiatan", "task"),
 ...
 [("ka","d1","","lurus"),("d1","d2","","lurus"),("d2","s1","","turun"),("s1","s2","","lurus"),
  ...
  ("a2","a4","ya","koridor"),("a4","s3","","turun"),("s3","a1","belum, menunggu\nasesor lain","koridor",-40),
